...
>
> **`RELAY_ENABLED` still defaults to `false`.** PR 5 (the one-press buttons) is part of Phase 1, and none of this has run against live Discord. The plan's two-account experiments come first. Flipping the default is one line, left for a deliberate decision.
