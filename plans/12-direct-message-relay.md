...
> **SHIPPED 2026-09-22 as `[Reply]` `[Not now]` `[Block sender]`**, on every relayed DM in both directions: a carried reply is a relay in its own right, and its buttons name *its* row. The sender's receipt has none. The owner chose the set. `[Not now]` takes the buttons away and tells the sender nothing, and the message can still be answered by replying to it until it lapses: not now, not never. `[Block sender]` is the one-press opt-out DECISIONS #23 promises. It blocks exactly what Apps → Block this sender blocks, in the same words (one `place_block` in `relay_router` serves all three block paths), works while the relay is switched off, and asks nothing first because `/relay unblock` undoes it. Departures from the text above, each found by reading discord.py 2.6.4's source and each pinned by a test:
> - **Registered at import, not in `on_ready`.** `on_ready` loads TTS (~17 s) before `add_cog`, so a press in that window matched nothing and vanished with no log line. On a reconnect, `add_cog` raises before any later line runs. `relay_router.install(client)` runs at module level. It needs no event loop, and registrations survive every READY. `on_ready` checks it again and repairs it loudly, and a test imports `main_discord` and inspects the real client.
> - **A relay view is stopped before it is sent.** discord.py stores every unfinished view it sends, and storing one registers its DynamicItem classes as a side effect. A missed registration would then have worked perfectly in testing and until the next restart, then failed on every DM ever sent. Stopped, the view serialises identically, and only the real registration makes the buttons work. The view holds only these three items and is never re-sent from a callback. Stopping a view that also holds an ordinary custom_id button unregisters its DynamicItem classes for the whole process, and re-sending the rebuilt view makes one press run two callbacks.
> - **Form submissions are read from the payload, in an `on_interaction` listener, not by the Modal.** discord.py keeps an open form only in memory: one submitted after a restart was dropped with a DEBUG line and nothing said to its author, and one closed without sending stayed in memory for good. The stored Modal is now inert. Every submission is handled identically, restart or not, and a test sends one to a bot that never opened a form.
> - **`[Reply]` reads the store on the event loop.** A form must be the press's first response, so the press cannot defer, and the worker pool can be full for minutes. It makes one indexed read, as `/relay block`'s autocomplete does. Everything is checked again when the form comes back: whether the relay is still open, forgotten or purged, whether the submitter is the recipient, and whether there are any words. The form's custom_id comes back from the client, so it names nothing until its row is shown to be the submitter's.
> - **Every press is anchored on its message.** The row is found by the message the button was pressed on, which Discord supplies, and must also carry the id in the custom_id. A lapsed, forgotten or purged relay says so and loses its buttons.
> - **Every press is answered by us.** The library answers nothing and logs nothing when a check fails; an exception is only logged, and the person sees "This interaction failed". Each handler catches its own failures. The listener is also the backstop for a relay button no class is registered for, which would otherwise break every historical DM without a trace: it logs an ERROR and tells the presser how else to reply or block.
> - Also: the form's length is clamped to Discord's 4000 whatever `RELAY_MAX_BODY_CHARS` says; the three templates are disjoint, since two matching patterns would both run; and a malformed relay id builds nothing, since the library builds with `re.match` but dispatches with `re.fullmatch`.
>
> A 10-agent adversarial review then found these, all fixed and pinned:
> - **A press could beat the DM's own bookkeeping.** `mark_sent` waits in the worker pool, so a DM can be on screen and pressed before its message is bound to its row. [Reply] then said the relay had lapsed and took its buttons away. A press now falls back to the id on the button itself, which Discord took off that very message, but only for a row still in flight. [Reply] opens the form, and [Block sender] blocks, naming the sender as the DM named them.
> - **A link change locked the real recipient out.** Every "was this delivered to you?" check compared only the resolved recipient, so an `IDENTITY_LINKS` entry added after delivery turned the recipient's own buttons, their native reply, the context menu and the block picker against them. One predicate, `relay_router.delivered_to`, now also accepts the account the DM was actually sent to, as `relay_store._mine` does for forget and export. That admits nobody new. **This reverses a PR 4 test**, which pinned that such a reply fell through to the agent. That was safe from misrouting, but it turned an answer meant for someone else into a turn with Fritz.
> - A form for someone else's relay reads exactly like one for no relay, so a guessed id says nothing about which relays exist. [Not now] no longer invites a reply while the relay is switched off: that reply would have gone to the agent. [Reply]'s failure copy no longer suggests replying to the message instead: when the store is the problem, that reply goes to the agent too. A [Block sender] whose button removal fails still acknowledges within three seconds.
> - The test double for an interaction response now records a refused second response as well as raising it. The handlers swallow every exception when answering, so a handler answering on the wrong channel had looked, to a count of calls, exactly like one answering once.
>
> **Unverified until the two-account test, and to check before `RELAY_ENABLED` flips:** that Discord accepts a form opened from a button in a bot DM; that ephemeral answers render in a bot DM; and what the client shows for an unanswered press.
...
> **SHIPPED 2026-09-21, following DECISIONS #22 over the text above.** `forget_relay` forgets every relayed message the person sent **or** received, and touches **no** block in either direction: blocks placed against them are other people's (anti-laundering), and blocks they placed survive so `/forget all` never re-arms a harasser. Those go only through `/relay forget-blocks` (landed in PR 3), which both the `/forget all` prompt and its result now name. The command's description no longer says "EVERYTHING". The prompt and the report say what is kept: delivered DMs stay in the recipients' Discord DMs, which no bot can unsend; the audit log keeps a text-free line per message for abuse handling; and a recipient can still block you from a message you sent (below).
> - **Forgetting must not reset a rate limit — so "delete" became "scrub, then delete".** The caps are counted off relay rows, and a first version that deleted them handed back the hour's quota: send until refused, `/forget all`, send again, without end, against the one control (the inbound cap) that stops a brigade. Rows older than the rate window (`RATE_WINDOW`, one hour) are deleted at once. Rows inside it keep only what the gate counts — the two ids, the status and the time — and lose the body, every name, and the routing key, and are closed and marked `forgotten`. The hourly purge deletes them once they leave the window. Scrubbed rows appear in no export, no count and no block picker, and `mark_sent` will not write the routing key or author line back onto a row scrubbed while its DM was in flight.
> - **A sender forgetting must not disarm the recipient's block**, extending DECISIONS #22's reasoning, that a privacy command must not be a safety regression, to the other party. A first version scrubbed everything the forgetter sent, so a harasser could relay, `/forget all`, and relay again, and each time the recipient's "Block this sender" said "not a message I carried to you" while `/relay block`'s picker came up empty. So a message the forgetter sent that was delivered, or is in flight, becomes a **block anchor** (`forgotten = 2`) rather than being scrubbed. It keeps the sending account, the author line the recipient was shown, and the DM id, until normal retention. The body and everything else go at once, and the anchor is never exported, counted or routed: a reply to it lapses. When the recipient forgets, the anchor goes too. An anchor whose message was never delivered goes with the scrubbed rows. **This is a judgement call made while implementing, not one of the recorded decisions; it is the one place where "forget me" keeps something, and the prompt says so.**
> - **Rows follow the account that wrote them.** Forget and export match the resolved identity **or** the literal account (`sender_account` / `recipient_account`, compared with `IS` so legacy NULLs do not poison `NOT`), so an `IDENTITY_LINKS` entry added or removed after the fact no longer strands rows that are neither forgotten nor exported.
> - **The count `/forget all` reports is what you could have seen**: everything you sent, and what was *delivered* to you. Counting refused attempts at you would reveal, as a number, what `/export` deliberately withholds.
> - **Identity resolves exactly once.** `privacy.forget_relay` and `export_relay` resolving on top of the store followed a second hop under chained `IDENTITY_LINKS` (A→B, B→C): A's own messages untouched and C's gone. The same double hop already existed for every other store, because `forget_all` and `export_user_data` resolved and then each operation resolved again. So `/forget all` from A deleted C's memories, conversation, schedules and workspace. Both entry points now hand every operation the id as given, and each resolves at its own entry point.
> - **`/export` follows PR 3's rule: only what you typed or were shown.** A sent message names the account you addressed. That needed a new `recipient_account` column, because `recipient_id` is resolved, and exporting it would have told a sender which main account the alt they wrote to belongs to. A received message names its sender by the author line you saw (`shown_as`). "Received" means *delivered*, so a refused attempt is never shown to its target, since that would say someone they blocked keeps trying. Your own refused attempts read identically whether a block or closed DMs refused them. Your blocks are included; blocks against you never are.
> - `_ForgetConfirmView` now reports every key `forget_all` returns, including `alias_dropped`. That key had been deleted on every `/forget all` and reported to nobody; a test now renders the real function's result, so a store added to `forget_all` cannot be left out of the report. The view also gained the `on_timeout` it always lacked.
> - **`reconcile_pending` closes only what a previous process left open**: reservations created before this process started, however young, become `failed`, which charges no one. The plan's "older than a grace window" was wrong twice. It missed a restart inside a minute, and because `on_ready` fires again on every gateway reconnect, it closed *live* reservations whose DM was still in flight, leaving a delivered relay permanently lapsed.
> - **`purge_expired` runs at boot and then hourly** as `_internal_relay_retention`, with `misfire_grace_time=None` and `coalesce=True`. A daily 03:30 job with APScheduler's default one-second grace was skipped whenever the loop was busy at that second, and never ran at all on a bot that restarts more often than daily. It deletes rows past `RELAY_RETENTION_DAYS` **unless they can still be answered** (delivered, unclosed, unexpired): deleting one of those turned the recipient's answer into a conversation with Fritz. If the reply window is configured longer than retention, the window wins and `validate_config` warns. Blocks are never purged: a block is a standing instruction, not a record. Nothing in `relay_housekeeping` can stop the bot coming up.
> - **A reply to a relay whose row is gone is told the exchange has lapsed**, and never reaches the agent. Forgetting, purging or reconciling removes the row a reply routes through, so the router now recognises Fritz's own relay embed by its footer (`relay_format.FOOTER_*`, `relay_kind`), and trusts only messages Fritz itself wrote. A reply to the sender's own receipt, which was never routable, is now told that it reaches no one, instead of becoming an agent turn carrying words meant for someone else. The footer prefixes are a format: changing one stops Fritz recognising every DM already sent with the old one.
> - `/export` writes UTF-8 rather than `\u` escapes (an emoji escaped is three times its size), and when it is too large it names the largest section and the command that trims it. A relay token taken from an old export no longer blocks through a scrubbed row, which had fallen back to the resolved sender and named the main account behind an alt. "Block this sender" on a relay whose row has gone now says so, instead of "not a message I carried to you".
>
> **`RELAY_ENABLED` still defaults to `false`.** Phase 1's code is complete with PR 5, but none of it has run against live Discord. The plan's two-account experiments come first. Flipping the default is one line, left for a deliberate decision.
